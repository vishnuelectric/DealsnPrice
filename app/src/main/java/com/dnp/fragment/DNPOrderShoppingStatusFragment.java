...
import com.dnp.adapter.OrderShoppingStatusAdapter;
import com.dnp.asynctask.Pending_amount;
import com.dnp.data.StaticData;

public class DNPOrderShoppingStatusFragment extends Fragment{
	View view;
	...
	TextView offer_layout,order_status_text;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		view=inflater.inflate(R.layout.activity_order_shopping_status, container, false);
		...
		order_status_text=(TextView) view.findViewById(R.id.order_status_text);
	...
	//	profile_icon.setImageDrawable(getResources().getDrawable(R.drawable.profile_active));
		...
		offer_layout.setOnClickListener(orderstatusListener);
	...
	OnClickListener statusListener=new OnClickListener() {
		...
		public void onClick(View v) {
			// TODO Auto-generated method stub
			//ArrayAdapter<String> adapter=new ArrayAdapter<String>(getActivity(), android.R.layout.simple_expandable_list_item_1, status);
			...
			status_list.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> arg0, View arg1,
					...
					order_status_text.setText(status[arg2]);
					...
					else if(status[arg2].equals("Confirmed")){
						for(int j=0;j<StaticData.account_detail.size();j++){
							if(StaticData.account_detail.get(j).getCashback_status().equals("2")){
								StaticData.order_filter_list.add(StaticData.account_detail.get(j));
							}
							type1="2";
						}
					}
					else if(status[arg2].equals("Rejected")){
						for(int k=0;k<StaticData.account_detail.size();k++){
							if(Integer.parseInt(StaticData.account_detail.get(k).getCashback_status())>2){
								StaticData.order_filter_list.add(StaticData.account_detail.get(k));
							}
							type1="3";
						}
					}
					OrderShoppingStatusAdapter ossadapter=new OrderShoppingStatusAdapter(getActivity(),type1);
					order_list.setAdapter(ossadapter);
					ossadapter.notifyDataSetChanged();
